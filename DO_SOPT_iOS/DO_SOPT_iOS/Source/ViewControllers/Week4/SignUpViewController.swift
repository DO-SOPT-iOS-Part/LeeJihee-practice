...
class SignUpViewController: UIViewController {
    ...
    var isUserNameExisted: Bool = false
    ...
    @objc private func registerButtonTap() {
        ...
        Task {
            do{
                let data = try await GetUserExistedService.shared.GetUserExisted(userName: userName)
                guard let data = data else { return }
                self.isUserNameExisted = data.isExist
            }
            if isUserNameExisted == false {
                Task {
                    do {
